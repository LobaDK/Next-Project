...
public class MockUserDataGenerator
{
    ...
    public static void GenerateMockUsers()
    {
        ...
        // Generate 80 DefaultUsers and 20 Managers for a realistic distribution
        GenerateUsersForRole(users, UserRoles.DefaultUser, 80, random, usedUsernames);
        GenerateUsersForRole(users, UserRoles.Manager, 20, random, usedUsernames);
        ...
        Console.WriteLine($"Generated {users.Count} mock users ({users.Count(u => u.Role == UserRoles.DefaultUser)} DefaultUsers, {users.Count(u => u.Role == UserRoles.Manager)} Managers) and saved to mocked_user_data.json");
    }

    private static void GenerateUsersForRole(List<MockedUser> users, UserRoles role, int count, Random random, HashSet<string> usedUsernames)
    {
