...
public interface IAuthenticationBridge
{
    ...
    /// </param>
    /// <returns>
    /// A tuple containing: the list of users in the requested page, a session identifier/continuation token for retrieving the next page (null or empty if none), and true if additional pages are available; otherwise, false.
    /// </returns>
    ...
    /// The type of the entity whose properties are to be inspected for AuthenticationMapping attributes.
    /// </typeparam>
    /// <returns>
    /// An array of entry names extracted from the AuthenticationMapping attributes applied to the properties of MappedEntity.
    /// </returns>
